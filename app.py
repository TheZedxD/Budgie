...
def make_treeview_resizable(tree: ttk.Treeview) -> None:
    ...
    def _on_configure(event: tk.Event) -> None:
        ...
        per_col = max(int(width / len(columns)), 20)
        for col in columns:
            tree.column(col, width=per_col)

    tree.bind("<Configure>", _on_configure)
...
class ThemeManager:
    ...
    def apply_theme(self, root):
        ...
        style.configure("TLabel", background=theme["label_bg"], foreground=theme["fg"])

        style.configure("TFrame", background=theme["frame_bg"], borderwidth=0)
        ...
        style.configure("TNotebook", background=theme["frame_bg"], borderwidth=1)
        ...
        style.configure("TPanedwindow", background=theme["frame_bg"])
...
class SavingsAccount:
    ...
    def from_dict(cls, data):
        return cls(
            data["name"], data.get("balance", 0.0), data.get("interest_rate", 0.0)
        )
...
class TaxCalculator:
    ...
    def calculate_federal_income_tax(
        annual_income, filing_status="single", allowances=1
    ):
        ...
        taxable_income = max(
            0, annual_income - standard_deduction - allowance_deduction
        )
    ...
    def calculate_virginia_state_tax(annual_income):
        ...
        brackets = [(3000, 0.02), (5000, 0.03), (17000, 0.05), (float("inf"), 0.0575)]
    ...
    def calculate_tax_breakdown(self):
        ...
        periods_per_year = (
            ...
            if self.frequency == "daily"
                ...
                else 26 if self.frequency == "bi-weekly" else 12
            )
        )
...
class CalendarWidget:
    def __init__(
        ...
        self.first_weekday = preferences.get("first_weekday", calendar.SATURDAY)
    ...
    def setup_calendar(self):
        ...
        ttk.Button(nav_frame, text="◀ Prev", command=self.previous_month).pack(
            side=tk.LEFT
        )
        ...
        ttk.Button(nav_frame, text="Next ▶", command=self.next_month).pack(
            side=tk.RIGHT
        )
        ttk.Button(nav_frame, text="Today", command=self.goto_current_month).pack(
        ...
        # Day headers with dynamic first weekday
        cal = calendar.Calendar(firstweekday=self.first_weekday)
        days = [calendar.day_name[d] for d in cal.iterweekdays()]
    ...
    def update_calendar(self):
        ...
        cal_data = self.calculator.get_calendar_data(
...
class DayDetailDialog:
    ...
    def setup_dialog(self):
        ...
        date_label = ttk.Label(
            main_frame,
            text=f"Transactions for {self.selected_date.strftime('%A, %B %d, %Y')}",
            font=("Arial", 12, "bold"),
        )
        ...
        scrollbar = ttk.Scrollbar(
            transactions_frame, orient=tk.VERTICAL, command=tree.yview
        )
        ...
        if not transactions:
            no_trans_label = ttk.Label(
                transactions_frame, text="No transactions scheduled for this day"
            )
            no_trans_label.grid(row=0, column=0, pady=20)
        else:
            for trans in transactions:
                type_display = (
                    "Income" if trans.transaction_type == "income" else "Expense"
                )
                amount_display = f"${trans.amount:,.2f}"

                color_tag = (
                    f"cat_{trans.category}"
...
class BudgieApp:
    def __init__(self, root):
        ...
        self.first_weekday = self.preferences.get("first_weekday", calendar.SATURDAY)
        calendar.setfirstweekday(self.first_weekday)
        ...
        self.root.after(
            1000, self.auto_refresh_on_startup
    ...
    def configure_scaling(self) -> None:
        """Adjust Tk scaling based on display DPI."""
        dpi = self.root.winfo_fpixels("1i")
        self.root.tk.call("tk", "scaling", dpi / 72)

    def run_in_thread(
        self, func: Callable[[], Any], callback: Callable[[], None]
    ) -> None:
        def worker() -> None:
            try:
                func()
    ...
    def toggle_week_start(self) -> None:
        """Toggle between Saturday and Sunday as the first weekday."""
        self.first_weekday = (
            calendar.SATURDAY if self.week_start_var.get() else calendar.SUNDAY
        )
        calendar.setfirstweekday(self.first_weekday)
        self.preferences.set("first_weekday", self.first_weekday)
        if hasattr(self, "calendar_widget"):
            self.calendar_widget.first_weekday = self.first_weekday
    ...
    def setup_menu(self):
        ...
        paycheck_menu.add_command(
            label="Add Paycheck", command=self.add_paycheck_dialog
        )
        paycheck_menu.add_command(
            label="Manage Paychecks", command=self.manage_paychecks
        )

        trans_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Transactions", menu=trans_menu)
        trans_menu.add_command(
            label="Add Transaction", command=self.add_transaction_dialog
        )
        trans_menu.add_command(
            label="Manage Transactions", command=self.manage_transactions
        )
        trans_menu.add_separator()
        trans_menu.add_command(
            label="Set Current Balance", command=self.set_balance_dialog
        )
        ...
        portfolio_menu.add_command(
            label="View Portfolio", command=self.show_crypto_portfolio
        )
        portfolio_menu.add_command(
            label="Add Crypto Holding", command=self.add_crypto_dialog
        )
        portfolio_menu.add_command(
            label="Refresh Prices", command=self.refresh_crypto_prices
        )

        savings_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Savings", menu=savings_menu)
        savings_menu.add_command(
            label="Manage Savings Accounts", command=self.manage_savings
        )

        view_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="View", menu=view_menu)
        self.week_start_var = tk.BooleanVar(
            value=self.first_weekday == calendar.SATURDAY
        )
        view_menu.add_checkbutton(
            label="Week starts on Saturday",
            variable=self.week_start_var,
            command=self.toggle_week_start,
        )

        tools_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Tools", menu=tools_menu)
        tools_menu.add_command(
            label="Monthly Analyzer", command=self.show_monthly_analyzer
        )
        tools_menu.add_command(
            label="Future Balance Projection", command=self.show_future_projection
        )
    ...
    def setup_main_interface(self):
        ...
        self.calendar_widget.get_widget().grid(row=0, column=0, sticky="nsew")
    ...
    def setup_stats_panel(self, parent):
        ...
        canvas = tk.Canvas(
            panel, borderwidth=0, highlightthickness=0, background=theme["frame_bg"]
        )
        ...
        balance_frame = ttk.LabelFrame(
            scrollable_frame, text="Account Status", padding="10"
        )
        balance_frame.pack(fill=tk.X, padx=5, pady=5)

        ttk.Label(
            balance_frame, text="Current Balance:", font=("Arial", 10, "bold")
        ).pack(anchor=tk.W)
        self.balance_label = ttk.Label(
            balance_frame, text=f"${self.current_balance:,.2f}", font=("Arial", 12)
        )
        self.balance_label.pack(anchor=tk.W, padx=(10, 0))

        # Crypto portfolio section
        ttk.Label(
            balance_frame, text="Crypto Portfolio:", font=("Arial", 10, "bold")
        ).pack(anchor=tk.W, pady=(10, 0))
        self.crypto_balance_label = ttk.Label(
            balance_frame, text="$0.00", font=("Arial", 12)
        )
        self.crypto_balance_label.pack(anchor=tk.W, padx=(10, 0))

        columns = ("Symbol", "Amount", "Value")
        self.portfolio_tree = ttk.Treeview(
            balance_frame, columns=columns, show="headings", height=5
        )
        for col in columns:
            self.portfolio_tree.heading(col, text=col)
            self.portfolio_tree.column(col, width=80)
        self.portfolio_tree.pack(fill=tk.X, padx=(10, 0), pady=(5, 0))
        make_treeview_resizable(self.portfolio_tree)

        # Total net worth
        ttk.Label(
            balance_frame, text="Total Net Worth:", font=("Arial", 10, "bold")
        ).pack(anchor=tk.W, pady=(10, 0))
        self.net_worth_label = ttk.Label(
            balance_frame, text="$0.00", font=("Arial", 12, "bold")
        )
        self.net_worth_label.pack(anchor=tk.W, padx=(10, 0))

        ttk.Button(
            balance_frame, text="Set Balance", command=self.set_balance_dialog
        ).pack(fill=tk.X, pady=(10, 5))
        ttk.Button(
            balance_frame, text="Refresh All Data", command=self.refresh_all_data
        ...
        # Paycheck actions
        paycheck_frame = ttk.LabelFrame(
            scrollable_frame, text="Paychecks", padding="10"
        )
        paycheck_frame.pack(fill=tk.X, padx=5, pady=5)

        ttk.Button(
            paycheck_frame, text="Add Paycheck", command=self.add_paycheck_dialog
        ).pack(fill=tk.X, pady=2)
        ttk.Button(
            paycheck_frame, text="Manage Paychecks", command=self.manage_paychecks
        ).pack(fill=tk.X, pady=2)
        ...
        ttk.Button(
            actions_frame, text="Add Transaction", command=self.add_transaction_dialog
        ).pack(fill=tk.X, pady=2)
        ttk.Button(
            actions_frame, text="Manage Transactions", command=self.manage_transactions
        ).pack(fill=tk.X, pady=2)
        ttk.Button(
            actions_frame, text="View Portfolio", command=self.show_crypto_portfolio
        ).pack(fill=tk.X, pady=2)
        ttk.Button(
            actions_frame, text="Monthly Analyzer", command=self.show_monthly_analyzer
        ).pack(fill=tk.X, pady=2)

        # Monthly summary
        self.summary_frame = ttk.LabelFrame(
            scrollable_frame, text="Monthly Summary", padding="10"
        )
    ...
    def update_monthly_summary(self, year, month):
        ...
        ttk.Label(
            self.summary_frame,
            text=f"{month_name} {year} Summary",
            font=("Arial", 10, "bold"),
        ).grid(row=0, column=0, columnspan=2, pady=(0, 10))

        income, expenses = self.calculate_monthly_summary(year, month)

        ttk.Label(self.summary_frame, text="Income:", font=("Arial", 9, "bold")).grid(
            row=1, column=0, sticky=tk.W
        )
        self.monthly_income_label = ttk.Label(
            self.summary_frame, text=f"${income:,.2f}", foreground="green"
        )
        self.monthly_income_label.grid(row=1, column=1, sticky=tk.W, padx=(10, 0))

        ttk.Label(self.summary_frame, text="Expenses:", font=("Arial", 9, "bold")).grid(
            row=2, column=0, sticky=tk.W
        )
        self.monthly_expenses_label = ttk.Label(
            self.summary_frame, text=f"${expenses:,.2f}", foreground="red"
        )
        self.monthly_expenses_label.grid(row=2, column=1, sticky=tk.W, padx=(10, 0))

        net = income - expenses
        ttk.Label(self.summary_frame, text="Net:", font=("Arial", 9, "bold")).grid(
            row=3, column=0, sticky=tk.W
        )
        net_color = "green" if net >= 0 else "red"
        self.monthly_net_label = ttk.Label(
            self.summary_frame, text=f"${net:+,.2f}", foreground=net_color
        )
    ...
    def refresh_calendar(self):
        ...
        self.update_monthly_summary(
            self.calendar_widget.current_year, self.calendar_widget.current_month
        )

        # Update crypto display
        self.update_crypto_display()

    def refresh_crypto_prices(self) -> None:
    ...
    def add_crypto_dialog(self):
        dialog = CryptoDialog(
            self.root, "Add Crypto Holding", self.preferences, self.theme_manager
        )
        if dialog.result:
    ...
    def add_paycheck_dialog(self):
        dialog = PaycheckDialog(
            self.root, "Add Paycheck", self.preferences, self.theme_manager
        )
        if dialog.result:
            paycheck = Paycheck(
    ...
    def manage_paychecks(self, year=None, month=None):
        ...
        ttk.Label(frame, text="Paycheck Management", font=("Arial", 12, "bold")).pack(
            pady=(0, 10)
        )
        ...
        for paycheck in self.calculator.paychecks:
            ...
            end_date_str = (
                paycheck.end_date.strftime("%Y-%m-%d") if paycheck.end_date else "None"
            )
            tree.insert(
                "",
                "end",
        ...
        def edit_selected():
            ...
            if selection:
                ...
                edit_dialog = EditPaycheckDialog(
                    self.dialog, paycheck, self.preferences, self.theme_manager
                )
                if edit_dialog.result:
                    ...
                    paycheck.filing_status = edit_dialog.result.get(
                        "filing_status", "single"
                    )
                    paycheck.allowances = edit_dialog.result.get("allowances", 1)
                    paycheck.health_insurance = edit_dialog.result.get(
                        "health_insurance", 0.0
                    )
                    paycheck.other_deductions = edit_dialog.result.get(
                        "other_deductions", 0.0
                    )
                    # Refresh display
                    self.dialog.destroy()
                ...
                messagebox.showwarning(
                    "No Selection", "Please select a paycheck to edit."
                )
        ...
        def delete_selected():
            ...
            if selection:
                ...
                if messagebox.askyesno(
                    "Confirm Delete", f"Delete paycheck for '{paycheck.job_name}'?"
                ):
                ...
                messagebox.showwarning(
                    "No Selection", "Please select a paycheck to delete."
                )

        button_frame = ttk.Frame(frame)
        button_frame.pack(fill=tk.X, pady=(10, 0))

        ttk.Button(button_frame, text="Edit Selected", command=edit_selected).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(button_frame, text="Delete Selected", command=delete_selected).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(
            button_frame,
            text="Add New Paycheck",
            command=lambda: [self.dialog.destroy(), self.add_paycheck_dialog()],
    ...
    def setup_tax_breakdown_tab(self, parent, tree):
        ...
        ttk.Label(
            parent,
            text="Select a paycheck from the list to view tax breakdown",
            font=("Arial", 12),
        ).pack(pady=20)

        self.breakdown_frame = ttk.LabelFrame(
            parent, text="Tax Breakdown", padding="10"
        )
    ...
    def update_tax_breakdown(self, tree, parent):
        ...
        ttk.Label(
            ...
            text=f"Paycheck Breakdown for {paycheck.job_name}",
            font=("Arial", 14, "bold"),
        ).grid(row=row, column=0, columnspan=2, pady=(0, 15))
        row += 1

        ttk.Label(
            self.breakdown_frame, text="Gross Pay:", font=("Arial", 11, "bold")
        ).grid(row=row, column=0, sticky=tk.W)
        ttk.Label(
            self.breakdown_frame,
            text=f"${breakdown['gross_pay']:.2f}",
            font=("Arial", 11),
        ).grid(row=row, column=1, sticky=tk.W, padx=(20, 0))
        row += 1

        ttk.Label(
            self.breakdown_frame, text="Deductions:", font=("Arial", 11, "bold")
        ...
        for label, amount in deductions:
            ttk.Label(self.breakdown_frame, text=f"  {label}:").grid(
                row=row, column=0, sticky=tk.W, padx=(20, 0)
            )
            ttk.Label(
                self.breakdown_frame, text=f"${amount:.2f}", foreground="red"
            ).grid(row=row, column=1, sticky=tk.W, padx=(20, 0))
            row += 1

        ttk.Label(
            self.breakdown_frame, text="Total Deductions:", font=("Arial", 11, "bold")
        ).grid(row=row, column=0, sticky=tk.W, pady=(10, 0))
        ttk.Label(
            self.breakdown_frame,
            text=f"${breakdown['total_deductions']:.2f}",
            font=("Arial", 11, "bold"),
            foreground="red",
        ).grid(row=row, column=1, sticky=tk.W, padx=(20, 0), pady=(10, 0))
        row += 1

        ttk.Label(
            self.breakdown_frame,
            text="Net Pay (Take-Home):",
            font=("Arial", 12, "bold"),
        ).grid(row=row, column=0, sticky=tk.W, pady=(10, 0))
        ttk.Label(
            self.breakdown_frame,
            text=f"${breakdown['net_pay']:.2f}",
            font=("Arial", 12, "bold"),
            foreground="green",
        ).grid(row=row, column=1, sticky=tk.W, padx=(20, 0), pady=(10, 0))
        row += 1
        ...
        annual_net = breakdown["net_pay"] * (
            52
            if paycheck.frequency == "weekly"
            else 26 if paycheck.frequency == "bi-weekly" else 12
        )

        ttk.Label(
            self.breakdown_frame, text="Annual Projections:", font=("Arial", 11, "bold")
        ).grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=(20, 5))
        row += 1

        ttk.Label(self.breakdown_frame, text="  Annual Gross:").grid(
            row=row, column=0, sticky=tk.W, padx=(20, 0)
        )
        ttk.Label(self.breakdown_frame, text=f"${annual_gross:,.2f}").grid(
            row=row, column=1, sticky=tk.W, padx=(20, 0)
        )
        row += 1

        ttk.Label(self.breakdown_frame, text="  Annual Net:").grid(
            row=row, column=0, sticky=tk.W, padx=(20, 0)
        )
        ttk.Label(
            self.breakdown_frame, text=f"${annual_net:,.2f}", foreground="green"
        ).grid(row=row, column=1, sticky=tk.W, padx=(20, 0))
    ...
    def show_paycheck_context_menu(self, event):
        ...
        menu.add_command(
            label="Edit Paycheck", command=lambda: self.edit_paycheck_from_tree(tree)
        )
        menu.add_command(
            label="Delete Paycheck",
            command=lambda: self.delete_paycheck_from_tree(tree),
        )
    ...
    def edit_paycheck_from_tree(self, tree):
        ...
        paycheck = self.calculator.paychecks[idx]
        edit_dialog = EditPaycheckDialog(
            tree.winfo_toplevel(), paycheck, self.preferences, self.theme_manager
        )
        if edit_dialog.result:
            # Update paycheck with new values
            paycheck.job_name = edit_dialog.result["job_name"]
    ...
    def add_transaction_dialog(self):
        dialog = TransactionDialog(
            self.root, "Add Transaction", self.preferences, self.theme_manager
        )
        if dialog.result:
            trans = Transaction(
                dialog.result["name"],
    ...
    def manage_transactions(self, year=None, month=None):
        ...
        ttk.Label(
            frame, text="Transaction Management", font=("Arial", 12, "bold")
        ).pack(pady=(0, 10))
        ...
        def edit_selected():
            ...
            if selection:
                ...
                edit_dialog = EditTransactionDialog(
                    dialog, transaction, self.preferences, self.theme_manager
                )
                if edit_dialog.result:
        ...
        def delete_selected():
            ...
            if selection:
                ...
                if messagebox.askyesno(
                    "Confirm Delete", f"Delete transaction '{trans.name}'?"
                ):
        ...
        ttk.Button(
            ...
            command=lambda: [dialog.destroy(), self.add_transaction_dialog()],
        ).pack(side=tk.LEFT, padx=(0, 5))
    ...
    def show_transaction_context_menu(self, event):
        ...
        menu.add_command(
            ...
            command=lambda: self.edit_transaction_from_tree(tree),
        )
        menu.add_command(
            label="Delete Transaction",
            command=lambda: self.delete_transaction_from_tree(tree),
        )
        menu.tk_popup(event.x_root, event.y_root)
    ...
    def edit_transaction_from_tree(self, tree):
        ...
        transaction = self.calculator.transactions[idx]
        edit_dialog = EditTransactionDialog(
            tree.winfo_toplevel(), transaction, self.preferences, self.theme_manager
        )
        if edit_dialog.result:
            # Update transaction with new values
            transaction.name = edit_dialog.result["name"]
    ...
    def manage_savings(self):
        ...
        ttk.Label(frame, text="Savings Accounts", font=("Arial", 12, "bold")).pack(
            pady=(0, 10)
        )
    ...
    def show_future_projection(self):
        ...
        ttk.Label(
            frame, text="Future Balance Projection", font=("Arial", 12, "bold")
        ).pack(pady=(0, 20))
        ...
        for i in range(12):
            ...
            future_balance = self.calculator.calculate_running_balance(
                self.current_balance, future_date
            )
    ...
    def show_monthly_analyzer(self):
        ...
        analyzer_dialog = MonthlyAnalyzerWindow(
            self.root, self.calculator, self.preferences, self.theme_manager, self
        )
    ...
    def export_report(self):
        ...
        try:
            ...
            with open(filename, "w") as f:
                ...
                f.write(
                    f"Total Net Worth: ${(self.current_balance + crypto_value):,.2f}\n\n"
                )
                ...
                if self.calculator.paychecks:
                    for paycheck in self.calculator.paychecks:
                        ...
                        f.write(
                            f"  Health Insurance: ${breakdown['health_insurance']:.2f}\n"
                        )
                        f.write(
                            f"  Other Deductions: ${breakdown['other_deductions']:.2f}\n"
                        )
                ...
                if self.calculator.crypto_portfolio.holdings:
                    ...
                    f.write(
                        f"Total Portfolio Value: ${portfolio.get_total_value():,.2f}\n"
                    )
                    f.write(f"Total Cost Basis: ${portfolio.get_total_cost():,.2f}\n")
                    f.write(f"Total P/L: ${portfolio.get_total_profit_loss():,.2f}\n")
                    f.write(
                        f"Total P/L %: {portfolio.get_total_profit_loss_percentage():.2f}%\n\n"
                    )
                    ...
                    for holding in portfolio.holdings:
                        ...
                        f.write(
                            f"    Current Value: ${holding.get_current_value():.2f}\n"
                        )
                        f.write(f"    P/L: ${holding.get_profit_loss():,.2f}\n")
                        f.write(
                            f"    P/L %: {holding.get_profit_loss_percentage():.2f}%\n\n"
                        )

                    if portfolio.last_updated:
                        f.write(
                            f"Last Updated: {portfolio.last_updated.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                        )
                else:
                ...
                income, expenses = self.calculate_monthly_summary(
                ...
                f.write("-" * 35 + "\n")
                category_totals = self.get_monthly_category_breakdown(
                    current_year, current_month
                )
            ...
            messagebox.showinfo(
                "Export Complete", f"Financial report exported to:\n{filename}"
            )
    ...
    def set_balance_dialog(self):
        ...
        ttk.Button(button_frame, text="Save", command=save_balance).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def save_data(self):
        data = {
            ...
            "savings_accounts": [
                acc.to_dict() for acc in self.calculator.savings_accounts
            ],
    ...
    def load_data(self):
        if os.path.exists(self.data_file):
            try:
                ...
                if crypto_data:
                    self.calculator.crypto_portfolio = CryptoPortfolio.from_dict(
                        crypto_data
                    )
    ...
    def new_budget(self):
        if messagebox.askyesno(
            "New Budget", "Create a new budget? All current data will be lost."
        ):
            self.calculator = BudgetCalculator()
            self.current_balance = 0.0
            self.refresh_calendar()
...
class PreferencesDialog:
    ...
    def setup_appearance_tab(self, notebook):
        ...
        ttk.Label(appearance_frame, text="Theme:", font=("Arial", 12, "bold")).pack(
        ...
        ttk.Radiobutton(
            theme_frame, text="Light Theme", variable=self.theme_var, value="light"
        ).pack(anchor=tk.W)
        ttk.Radiobutton(
            theme_frame, text="Dark Theme", variable=self.theme_var, value="dark"
        ).pack(anchor=tk.W, pady=(5, 0))

        ttk.Label(
            theme_frame, text="(Requires restart to fully apply)", font=("Arial", 8)
        ).pack(anchor=tk.W, pady=(5, 0))

        ttk.Label(
            appearance_frame, text="Display Options:", font=("Arial", 12, "bold")
        ).pack(anchor=tk.W, pady=(20, 10))

        self.show_weekends_var = tk.BooleanVar(
            value=self.preferences.get("show_weekends", True)
        )
        self.show_weekends_var.trace("w", self.on_change)
        ttk.Checkbutton(
            appearance_frame,
            text="Show weekends in calendar",
            variable=self.show_weekends_var,
        ).pack(anchor=tk.W)

        self.show_cents_var = tk.BooleanVar(
            value=self.preferences.get("show_cents", True)
        )
        self.show_cents_var.trace("w", self.on_change)
        ttk.Checkbutton(
            appearance_frame, text="Show cents in amounts", variable=self.show_cents_var
        ).pack(anchor=tk.W, pady=(5, 0))
    ...
    def setup_general_tab(self, notebook):
        ...
        ttk.Label(
            general_frame, text="Currency Settings:", font=("Arial", 12, "bold")
        ).pack(anchor=tk.W, pady=(0, 10))

        currency_frame = ttk.Frame(general_frame)
        currency_frame.pack(anchor=tk.W, pady=(0, 20))

        ttk.Label(currency_frame, text="Currency Symbol:").grid(
        ...
        currency_entry = ttk.Entry(
            currency_frame, textvariable=self.currency_var, width=5
        )
        currency_entry.grid(row=0, column=1, padx=(10, 0), sticky=tk.W)

        ttk.Label(currency_frame, text="Decimal Places:").grid(
            row=1, column=0, sticky=tk.W, pady=(10, 0)
        )
        self.decimal_var = tk.StringVar(
            value=str(self.preferences.get("decimal_places", 2))
        )
        self.decimal_var.trace("w", self.on_change)
        decimal_combo = ttk.Combobox(
            currency_frame, textvariable=self.decimal_var, width=5
        )
        ...
        ttk.Label(general_frame, text="Date Format:", font=("Arial", 12, "bold")).pack(
            anchor=tk.W, pady=(20, 10)
        )

        self.date_format_var = tk.StringVar(
            value=self.preferences.get("date_format", "%Y-%m-%d")
        )
        self.date_format_var.trace("w", self.on_change)
        date_combo = ttk.Combobox(
            general_frame, textvariable=self.date_format_var, width=20
        )
        ...
        ttk.Label(
            general_frame, text="File Settings:", font=("Arial", 12, "bold")
        ).pack(anchor=tk.W, pady=(20, 10))

        self.auto_save_var = tk.BooleanVar(
            value=self.preferences.get("auto_save", True)
        )
        self.auto_save_var.trace("w", self.on_change)
        ttk.Checkbutton(
            general_frame, text="Auto-save changes", variable=self.auto_save_var
        ).pack(anchor=tk.W)
    ...
    def setup_colors_tab(self, notebook):
        ...
        ttk.Label(
            colors_frame, text="Category Colors:", font=("Arial", 12, "bold")
    ...
    def setup_buttons(self):
        ...
        ttk.Button(button_frame, text="Apply", command=self.apply_preferences).pack(
...
class CryptoPortfolioWindow:
    ...
    def setup_window(self):
        ...
        ttk.Label(
            header_frame, text="Cryptocurrency Portfolio", font=("Arial", 16, "bold")
        ).pack(side=tk.LEFT)
        ttk.Button(
            header_frame, text="Refresh Prices", command=self.refresh_prices
        ).pack(side=tk.RIGHT)
        ttk.Button(header_frame, text="Add Holding", command=self.add_holding).pack(
            side=tk.RIGHT, padx=(0, 10)
        )

        summary_frame = ttk.LabelFrame(
            main_frame, text="Portfolio Summary", padding="10"
        )
        ...
        scrollbar = ttk.Scrollbar(
            holdings_frame, orient=tk.VERTICAL, command=self.tree.yview
        )
    ...
    def add_holding(self):
        dialog = CryptoDialog(
            self.dialog, "Add Crypto Holding", self.preferences, self.theme_manager
        )
        if dialog.result:
            holding = CryptoHolding(
                dialog.result["symbol"],
    ...
    def edit_holding(self, event):
        ...
        edit_dialog = EditCryptoDialog(
            self.dialog, holding, self.preferences, self.theme_manager
        )
        if edit_dialog.result:
...
class CryptoDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Label(
            frame, text="Add Cryptocurrency Holding", font=("Arial", 12, "bold")
        ).grid(row=0, column=0, columnspan=2, pady=(0, 20))

        ttk.Label(frame, text="Symbol (e.g., BTC):").grid(
        ...
        ttk.Label(frame, text="Name (e.g., Bitcoin):").grid(
        ...
        ttk.Label(frame, text="Purchase Price ($):").grid(
            row=4, column=0, sticky=tk.W, pady=5
        )
        self.price_var = tk.StringVar()
        ttk.Entry(frame, textvariable=self.price_var, width=30).grid(
            row=4, column=1, pady=5, padx=(10, 0)
        )

        ttk.Label(frame, text="Purchase Date:").grid(
            row=5, column=0, sticky=tk.W, pady=5
        )
        self.date_var = tk.StringVar(value=datetime.now().strftime("%Y-%m-%d"))
        ttk.Entry(frame, textvariable=self.date_var, width=30).grid(
            row=5, column=1, pady=5, padx=(10, 0)
        )

        button_frame = ttk.Frame(frame)
        button_frame.grid(row=6, column=0, columnspan=2, pady=20)

        ttk.Button(button_frame, text="Add Holding", command=self.save_holding).pack(
            side=tk.LEFT, padx=5
        )
...
class EditCryptoDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Label(
            frame, text="Edit Cryptocurrency Holding", font=("Arial", 12, "bold")
        ).grid(row=0, column=0, columnspan=2, pady=(0, 20))
        ...
        self.date_var = tk.StringVar(
            value=self.holding.purchase_date.strftime("%Y-%m-%d")
        )
        ttk.Entry(frame, textvariable=self.date_var, width=30).grid(
            row=5, column=1, pady=5, padx=(10, 0)
        )
        ...
        ttk.Label(frame, text="Current Price:").grid(
            row=6, column=0, sticky=tk.W, pady=5
        )
        current_price_text = (
            f"${self.holding.current_price:.2f}"
            if self.holding.current_price > 0
            else "Not updated"
        )
        ttk.Label(frame, text=current_price_text, foreground="blue").grid(
            row=6, column=1, sticky=tk.W, padx=(10, 0), pady=5
        )

        button_frame = ttk.Frame(frame)
        button_frame.grid(row=7, column=0, columnspan=2, pady=20)

        ttk.Button(button_frame, text="Save Changes", command=self.save_holding).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=self.dialog.destroy).pack(
            side=tk.LEFT
...
class PaycheckDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Button(button_frame, text="Save Paycheck", command=self.save_paycheck).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=self.dialog.destroy).pack(
            side=tk.LEFT
    ...
    def setup_basic_tab(self, frame):
        ttk.Label(frame, text="Job/Employer Name:").grid(
        ...
        ttk.Label(frame, text="Hourly Rate ($):").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.hourly_rate_var = tk.StringVar()
        ttk.Entry(frame, textvariable=self.hourly_rate_var, width=30).grid(
            row=1, column=1, pady=5, padx=(10, 0)
        )

        ttk.Label(frame, text="Hours per Week:").grid(
            row=2, column=0, sticky=tk.W, pady=5
        )
        self.hours_var = tk.StringVar(value="40")
        ttk.Entry(frame, textvariable=self.hours_var, width=30).grid(
            row=2, column=1, pady=5, padx=(10, 0)
        )

        ttk.Label(frame, text="Pay Frequency:").grid(
        ...
        ttk.Label(frame, text="First Paycheck Date:").grid(
            row=4, column=0, sticky=tk.W, pady=5
        )
        self.date_var = tk.StringVar(value=datetime.now().strftime("%Y-%m-%d"))
        ttk.Entry(frame, textvariable=self.date_var, width=30).grid(
        ...
        ttk.Label(frame, text="Last Paycheck (optional):").grid(
            row=5, column=0, sticky=tk.W, pady=5
        )
        self.end_date_var = tk.StringVar()
        ttk.Entry(frame, textvariable=self.end_date_var, width=30).grid(
            row=5, column=1, pady=5, padx=(10, 0)
        )

    def setup_tax_tab(self, frame):
        ttk.Label(frame, text="Tax Information", font=("Arial", 12, "bold")).grid(
            row=0, column=0, columnspan=2, pady=(0, 15)
        )

        ttk.Label(frame, text="Filing Status:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.filing_status_var = tk.StringVar(value="single")
        filing_combo = ttk.Combobox(
            frame, textvariable=self.filing_status_var, width=27
        )
        filing_combo["values"] = (
            "single",
            "married_joint",
        ...
        ttk.Label(frame, text="Tax Allowances:").grid(
            row=2, column=0, sticky=tk.W, pady=5
        )
        self.allowances_var = tk.StringVar(value="1")
        ttk.Entry(frame, textvariable=self.allowances_var, width=30).grid(
            row=2, column=1, pady=5, padx=(10, 0)
        )

        ttk.Label(frame, text="Deductions", font=("Arial", 12, "bold")).grid(
            row=3, column=0, columnspan=2, pady=(20, 15)
        )

        ttk.Label(frame, text="Health Insurance (per pay):").grid(
            row=4, column=0, sticky=tk.W, pady=5
        )
        self.health_insurance_var = tk.StringVar(value="0.00")
        ttk.Entry(frame, textvariable=self.health_insurance_var, width=30).grid(
            row=4, column=1, pady=5, padx=(10, 0)
        )

        ttk.Label(frame, text="Other Deductions (per pay):").grid(
            row=5, column=0, sticky=tk.W, pady=5
        )
        self.other_deductions_var = tk.StringVar(value="0.00")
        ttk.Entry(frame, textvariable=self.other_deductions_var, width=30).grid(
            row=5, column=1, pady=5, padx=(10, 0)
        )

        # Add trace for real-time preview updates
        for var in [
            self.hourly_rate_var,
            self.hours_var,
            self.frequency_var,
            self.filing_status_var,
            self.allowances_var,
            self.health_insurance_var,
            self.other_deductions_var,
        ]:
            var.trace("w", self.update_preview)

    def setup_preview_tab(self, frame):
        ttk.Label(frame, text="Paycheck Preview", font=("Arial", 14, "bold")).pack(
            pady=(0, 20)
        )

        self.preview_frame = ttk.LabelFrame(
            frame, text="Calculated Paycheck", padding="15"
        )
        ...
        ttk.Label(
            self.preview_frame, text="Enter paycheck details to see preview"
        ).pack(pady=20)
    ...
    def update_preview(self, *args):
        try:
            ...
            if rate > 0 and hours > 0:
                ...
                row = 0
                ttk.Label(
                    self.preview_frame, text="Gross Pay:", font=("Arial", 11, "bold")
                ).grid(row=row, column=0, sticky=tk.W)
                ttk.Label(
                    self.preview_frame,
                    text=f"${breakdown['gross_pay']:.2f}",
                ...
                ttk.Label(
                    self.preview_frame, text="Deductions:", font=("Arial", 11, "bold")
                ).grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=(10, 5))
                row += 1
                ...
                for label, amount in deductions:
                    ttk.Label(self.preview_frame, text=f"  {label}:").grid(
                        row=row, column=0, sticky=tk.W, padx=(20, 0)
                    )
                    ttk.Label(
                        self.preview_frame, text=f"${amount:.2f}", foreground="red"
                    ).grid(row=row, column=1, sticky=tk.W, padx=(20, 0))
                    row += 1
                ...
                annual_net = breakdown["net_pay"] * (
                    52
                    if frequency == "weekly"
                    else 26 if frequency == "bi-weekly" else 12
                )

                ttk.Label(
                    self.preview_frame,
                    text="Annual Estimates:",
                    font=("Arial", 11, "bold"),
                ).grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=(20, 5))
                row += 1

                ttk.Label(self.preview_frame, text="  Annual Gross:").grid(
                    row=row, column=0, sticky=tk.W, padx=(20, 0)
                )
                ttk.Label(self.preview_frame, text=f"${annual_gross:,.2f}").grid(
                    row=row, column=1, sticky=tk.W, padx=(20, 0)
                )
                row += 1

                ttk.Label(self.preview_frame, text="  Annual Net:").grid(
                    row=row, column=0, sticky=tk.W, padx=(20, 0)
                )
                ttk.Label(
                    self.preview_frame, text=f"${annual_net:,.2f}", foreground="green"
                ).grid(row=row, column=1, sticky=tk.W, padx=(20, 0))

            else:
                ttk.Label(
                    self.preview_frame,
                    text="Enter hourly rate and hours to see preview",
                ).pack(pady=20)

        except (ValueError, AttributeError):
            ttk.Label(self.preview_frame, text="Please check your input values").pack(
                pady=20
            )
...
class EditPaycheckDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Button(button_frame, text="Save Changes", command=self.save_paycheck).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=self.dialog.destroy).pack(
            side=tk.LEFT
    ...
    def setup_basic_tab(self, frame):
        ...
        self.date_var = tk.StringVar(
            value=self.paycheck.start_date.strftime("%Y-%m-%d")
        )
        ttk.Entry(frame, textvariable=self.date_var, width=30).grid(
            row=4, column=1, pady=5, padx=(10, 0)
        )
        ...
        end_date_value = (
            self.paycheck.end_date.strftime("%Y-%m-%d")
            if self.paycheck.end_date
            else ""
        )
    ...
    def setup_tax_tab(self, frame):
        ...
        self.filing_status_var = tk.StringVar(
            value=getattr(self.paycheck, "filing_status", "single")
        )
        filing_combo = ttk.Combobox(
            frame, textvariable=self.filing_status_var, width=27
        )
        ...
        self.allowances_var = tk.StringVar(
            value=str(getattr(self.paycheck, "allowances", 1))
        )
        ttk.Entry(frame, textvariable=self.allowances_var, width=30).grid(
            row=2, column=1, pady=5, padx=(10, 0)
        )
        ...
        self.health_insurance_var = tk.StringVar(
            value=str(getattr(self.paycheck, "health_insurance", 0.0))
        )
        ttk.Entry(frame, textvariable=self.health_insurance_var, width=30).grid(
            row=4, column=1, pady=5, padx=(10, 0)
        )
        ...
        self.other_deductions_var = tk.StringVar(
            value=str(getattr(self.paycheck, "other_deductions", 0.0))
        )
        ttk.Entry(frame, textvariable=self.other_deductions_var, width=30).grid(
            row=5, column=1, pady=5, padx=(10, 0)
        )
    ...
    def setup_preview_tab(self, frame):
        ...
        self.preview_frame = ttk.LabelFrame(
            frame, text="Updated Paycheck", padding="15"
        )
        self.preview_frame.pack(fill=tk.BOTH, expand=True)
...
class EditTransactionDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Label(frame, text="Edit Transaction", font=("Arial", 12, "bold")).grid(
            row=0, column=0, columnspan=2, pady=(0, 20)
        )

        # Pre-fill with existing transaction data
        ttk.Label(frame, text="Transaction Name:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        ...
        self.date_var = tk.StringVar(
            value=self.transaction.start_date.strftime("%Y-%m-%d")
        )
        ttk.Entry(frame, textvariable=self.date_var, width=30).grid(
            row=6, column=1, pady=5, padx=(10, 0)
        )
        ...
        end_date_value = (
            self.transaction.end_date.strftime("%Y-%m-%d")
            if self.transaction.end_date
            else ""
        )
        self.end_date_var = tk.StringVar(value=end_date_value)
        ttk.Entry(frame, textvariable=self.end_date_var, width=30).grid(
            row=7, column=1, pady=5, padx=(10, 0)
        )
        ...
        ttk.Button(
            button_frame, text="Save Changes", command=self.save_transaction
...
class TransactionDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Button(button_frame, text="Save", command=self.save_transaction).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=self.dialog.destroy).pack(
            side=tk.LEFT
...
class MonthlyAnalyzerWindow:
    ...
    def setup_window(self):
        ...
        ttk.Button(header_frame, text="◀ Prev Month", command=self.previous_month).pack(
            side=tk.LEFT
        )
        ...
        ttk.Button(header_frame, text="Next Month ▶", command=self.next_month).pack(
            side=tk.RIGHT
        )
        ttk.Button(
            header_frame, text="Current Month", command=self.goto_current_month
    ...
    def create_bar_chart(self, parent, category_data):
        ...
        bar_spacing = 10
        total_bar_width = (
            len(sorted_categories) * (bar_width + bar_spacing) - bar_spacing
        )
        start_x = margin_left + max(
            0, (chart_width - margin_left - total_bar_width) // 2
        )
        ...
        for i, (category, amount) in enumerate(sorted_categories):
            ...
            bar_height = (
                (amount / max_amount) * (chart_height - margin_bottom)
                if max_amount > 0
                else 0
            )
            ...
            for j in range(int(bar_height)):
                ...
                if j % 2 == 0:  # Only draw every other line for performance
                    canvas.create_line(
                        x, y + j, x + bar_width, y + j, fill=color, width=2
                    )
            ...
            canvas.create_text(
                x + bar_width / 2,
                chart_height - margin_bottom + 20,
                text=category.title()[:10],
                font=("Arial", 9),
                anchor="n",
            )
    ...
    def create_projections_chart(self):
        ...
        balance_range = (
            max_balance - min_balance if max_balance != min_balance else 1000
        )
        ...
        for i in range(6):
            ...
            canvas.create_line(
                margin, y, chart_width - margin, y, fill="#E0E0E0", width=1
            )
            balance_label = max_balance - (i * balance_range // 5)
            canvas.create_text(
                margin - 10,
                y,
                text=f"${balance_label:,.0f}",
                font=("Arial", 8),
                anchor="e",
            )
        ...
        for i, (proj_date, balance) in enumerate(projections):
            ...
            y = margin + ((max_balance - balance) / balance_range) * (
                chart_height - margin
            )
            points.extend([x, y])
        ...
        if min_balance < 0:
            zero_y = margin + ((max_balance - 0) / balance_range) * (
                chart_height - margin
            )
            canvas.create_line(
                margin,
    ...
    def create_5year_outlook(self):
        ...
        max_balance = max(proj[1] for proj in projections)
        balance_range = (
            max_balance - min_balance if max_balance != min_balance else 10000
        )
        ...
        for year in range(6):
            ...
            canvas.create_line(
                x, margin, x, chart_height - margin, fill="#E0E0E0", width=1
            )
            year_label = self.current_year + year
            canvas.create_text(
                x,
                chart_height - margin + 10,
                text=str(year_label),
                font=("Arial", 9),
                anchor="n",
            )
        ...
        for i, (proj_date, balance) in enumerate(projections):
            ...
            if proj_date.month == 1 and proj_date.day == 1:
                canvas.create_oval(
                    x - 3, y - 3, x + 3, y + 3, fill="blue", outline="black"
                )
    ...
    def create_overview_chart(self, income, expenses):
        ...
        # Income bar
        income_height = (
            (income / max_amount) * (chart_height - margin) if max_amount > 0 else 0
        )
        ...
        # Expenses bar
        expense_height = (
            (expenses / max_amount) * (chart_height - margin) if max_amount > 0 else 0
        )
    ...
    def create_summary(self, category_data, income, expenses):
        ...
        stats_frame = ttk.LabelFrame(
            self.summary_frame, text="Key Statistics", padding="15"
        )
        ...
        stats = [
            ...
            (
                ...
                f"${(expenses / len(category_data) if category_data else 0):,.2f}",
                "blue",
            ),
            (
        ...
        for i, (label, value, color) in enumerate(stats):
            ...
            ttk.Label(
                stats_frame, text=value, font=("Arial", 10), foreground=color
            ).grid(row=i, column=1, sticky=tk.W, padx=(20, 0), pady=5)

        # Category breakdown list
        if category_data:
            category_frame = ttk.LabelFrame(
                self.summary_frame, text="Detailed Breakdown", padding="15"
            )
            ...
            tree = ttk.Treeview(
                category_frame, columns=columns, show="headings", height=10
            )
            ...
            scrollbar = ttk.Scrollbar(
                category_frame, orient=tk.VERTICAL, command=tree.yview
            )
            tree.configure(yscrollcommand=scrollbar.set)
            ...
            for category, amount in sorted_categories:
                ...
                tree.insert(
                    "",
                    "end",
                    values=(category.title(), f"${amount:,.2f}", f"{percentage:.1f}%"),
                )
